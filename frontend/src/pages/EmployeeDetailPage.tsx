          ...
          <Paper variant="outlined" sx={{ p: 3 }}>
            <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "flex-start" }}>
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">{employee.jobTitle} &middot; {employee.department}</Typography>
              </Box>
              <Chip label={titleCase(employee.status)} color={employee.status === "ACTIVE" ? "success" : "default"} size="small" />
            </Stack>
            ...
            {employee.status === "ACTIVE" && (
              <Button sx={{ mt: 3 }} color="error" variant="outlined" size="small"
        ...
        <Grid size={{ xs: 12, md: 7 }}>
          <Paper variant="outlined" sx={{ p: 3, mb: 3 }}>
            <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "center", mb: 2 }}>
              ...
              {!editingSalary && (
                <Button size="small" onClick={() => { setEditingSalary(true); setNewSalary(String(employee.baseSalaryAnnual)); }}>
                  Update Salary
                </Button>
              )}
            </Stack>
            ...
            {editingSalary && (
              <Box sx={{ mt: 3 }}>
                ...
                <Stack spacing={2}>
                  ...
                  />
                  <TextField select label="Reason" size="small" value={changeReason} onChange={(e) => setChangeReason(e.target.value)}>
                    {(metaQuery.data?.changeReasons ?? ["MERIT_INCREASE", "PROMOTION", "MARKET_ADJUSTMENT", "CORRECTION"])
                      .filter((r) => r !== "INITIAL_HIRE")
                      .map((r) => <MenuItem key={r} value={r}>{titleCase(r)}</MenuItem>)}
                  </TextField>
                  <TextField label="Changed by" size="small" value={changedBy} onChange={(e) => setChangedBy(e.target.value)} />
                  <Stack direction="row" spacing={1}>
                    <Button variant="contained" onClick={submitSalaryChange} disabled={updateMutation.isPending}>
                      Save Change
                    </Button>
                    <Button onClick={() => { setEditingSalary(false); setFormError(null); }}>Cancel</Button>
                  </Stack>
                </Stack>
              </Box>
            )}
              ...
              <TableBody>
                {historyQuery.data?.map((h) => (
                  <TableRow key={h.id}>
                    ...
                    <TableCell><Chip label={titleCase(h.changeReason)} size="small" variant="outlined" /></TableCell>
                    <TableCell align="right">{h.previousSalary !== null ? formatUsd(h.previousSalary) : "—"}</TableCell>
